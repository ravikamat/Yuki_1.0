...
Write-Host "========================================================="
Write-Host " YUKI v1.0 -- P0 Unified Semantic Layer Live Chat Suite "
Write-Host "========================================================="

foreach ($idx in 0..($prompts.Length - 1)) {
    $prompt = $prompts[$idx]
    Write-Host "`n---------------------------------------------------------"
    Write-Host "Test Case $($idx + 1): '$prompt'"
    Write-Host "---------------------------------------------------------"

    $inputContent = "$prompt`nexit`n"
    ...
    # Extract YUKI response lines
    $lines = $output -split "`r?\n"
    $responseLines = $lines | Where-Object { $_ -and $_ -notmatch "^YUKI>" -and $_ -notmatch "^\s*$" -and $_ -notmatch "^\[" }
    if ($responseLines) {
        Write-Host "YUKI Output:" -ForegroundColor Green
...
Write-Host "`n========================================================="
Write-Host " P0 Live Chat Test Suite Complete "
Write-Host "========================================================="
