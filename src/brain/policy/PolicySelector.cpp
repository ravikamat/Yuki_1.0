...
#include "LearnedEnsemblePolicy.h"
...
PolicySelector::~PolicySelector() = default;
...
void PolicySelector::setSelfModel(yuki::self::SelfModel* ptr) {
    self_model_.reset(ptr);
}

void PolicySelector::setValenceArousalModel(yuki::emotion::ValenceArousalModel* ptr) {
...
float PolicySelector::computeActionRiskAdjustedThreshold(float baseThreshold, float riskAggregate) const {
    float adjusted = baseThreshold * (1.0f + riskAggregate * 0.75f);
    return std::clamp(adjusted, THRESHOLD_MIN, THRESHOLD_MAX);
}

bool PolicySelector::requiresHumanApprovalForAction(const std::string& actionType, float riskScore) const {
...
bool PolicySelector::applyEnsembleAdvisory(PolicySelection& result,
                                            float entropy,
...
void PolicySelector::setSystemController(yuki::system::SystemController* ptr) {
    system_controller_.reset(ptr);
}

bool PolicySelector::requiresSystemCapability(uint32_t action_type) const {
